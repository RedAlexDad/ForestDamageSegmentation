...
help:
	...
	@echo "  DATA_DIR=путь           - Путь к данным (по умолчанию: data/train)"
	@echo "  MODEL_DIR=путь         - Путь для сохранения моделей (по умолчанию: models)"
	@echo "  EPOCHS=число           - Количество эпох обучения (по умолчанию: 50)"
	@echo "  BATCH_SIZE=число        - Размер батча (по умолчанию: 8)"
	@echo "  LEARNING_RATE=число  - Скорость обучения (по умолчанию: 0.001)"
	@echo ""
...
lint:
	...
	@$(PYTHON) -m pylint src/ --disable=R,C --output-format=text || true
